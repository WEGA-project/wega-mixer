...
---
### Что потребуется купить(ардуино, драйвера, АЦП ...) <a name="what_to_buy"></a>
...
---
### Подключение по схеме компонентов для миксера <a name="connections"></a>
...
**Особенность Windows**
```
Если vscode на windows 10 не видит и не может сам подключить библиотеки из проекта в main.cpp онлайн, можно воспользоваться советом с форумов и сделать следующее:
1. Не закрывая текущий проект  - открыть новое окно (File-New Window)
2. Снова открыть папку проекта (File-Open Folder...) и выбрать повторно main.cpp. Проблема должна исчезнуть и можно старое окно закрыть
```
...
Варианты как залить прошивку описаны здесь: [Прошивка по Wi-Fi](#ota) или через [консоль](#usb) 

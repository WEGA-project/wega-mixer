...
Как только процесс загрузки закончится веб сервер должен быть запущен, IP адрес, который получила плата высветится на экране.
Кроме того, для операционных систем поддерживающих технологию mDNS (все системы и Windows начиная с версии 10) можно подключиться по адресу http://mixer.local.

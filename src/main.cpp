...
//   if (valueToPump > 0.3)
//   { // если быстро качать не много то не начинать даже
//     while ((valueToPump = goal[n] - curvol[n] - dropTreshold) > 0)
//     {
//       if (valueToPump > 0.2)
//         valueToPump = valueToPump / 2;             // качать по половине от остатка
...
//       curvol[n] = rawToUnits(readScalesWithCheck(128));
//       if (workedTime > 200 && curvol[n] - prevValue > 0.15)
//         performance = max(performance, (curvol[n] - prevValue) / workedTime);
//       server.handleClient();
//       sendReportUpdate();
//     }
//   }
